...
void turn(float angle) { // use +90 turn right and -90 to turn left

	float compassTarget, compassNow; //used for turning and configuring compass
	float error;

	compassNow = SensorValue(compass);
	compassTarget = (compassNow + angle); // in degrees

	compassNow = 10;
	compassTarget = compassTarget+90.0;

	//error = 180.0/PI*atan2(sinDegrees(compassTarget-compassNow),cosDegrees(compassTarget-compassNow));

	error = 180/PI*atan2(sinDegrees(compassTarget-compassNow),cosDegrees(compassTarget-compassNow));

	while(true || abs(error)>2.0){

		motor[leftWheel]  =  sgn(error)*(20.0*error+5.0);
		motor[rightWheel] = -sgn(error)*(20.0*error+5.0);

				motor[leftWheel]  =  0;
		motor[rightWheel] = 0;

		compassNow = SensorValue(compass);
		//error = 180.0/PI*atan2(sinDegrees(compassTarget)-sinDegrees(compassNow),cosDegrees(compassTarget)-cosDegrees(compassNow));
		nxtDisplayString(1, "%03d %03d %3.1f", (int)compassNow, (int)compassTarget, error);
	}
