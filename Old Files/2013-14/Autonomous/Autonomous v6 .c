...
int leftReadingHome;											// home values...
int rightReadingHome;
int strengthHome;
...
void seek_beacon(){									 //search for IR beacon and stop in front of it...
	...
	leftReadingHome  = HTIRS2readACDir(irLeft);		// store the location at the home position
	rightReadingHome = HTIRS2readACDir(irRight);
	...
	strengthHome = acc;														// the strength at the home position
...
void seek_home(){									  //try to go back home backwards using the original values...
	...
	while(!flag)
	{
		...
		if(strength>strengthHome)										 // keep moving while we are too close to the IR beacon
			speed = 12;

		// CALIBRATE -- (6,4) are the target values for the distance, but it may need to be calibrated in the real field.

		leftReading  = HTIRS2readACDir(irLeft)  - leftReadingHome;		// compute error for left and right eyes
		rightReading = HTIRS2readACDir(irRight) - rightReadingHome;

		// compute translation and rotation component of the movement depending on the errors
		...
		if((leftReading == 0 && rightReading == 0 && strength<=1.15*strengthHome) || (strength<strengthHome) ) {		//if the errors are zero and we reached the target
			...
			motor[leftWheel] = speed*(t-0.6*r);		// keep a history of our movements commands
			motor[rightWheel] = speed*(t+0.6*r);
...
//	waitForStart(); 		// Wait for the beginning of autonomous phase.
	...
	seek_home();			  // try to go back using IR
	...
	wait10Msec(200); 				//CALIBRATE time to make sure we get on the ramp
